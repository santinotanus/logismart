...
Para construir una arquitectura robusta, transaccional y modular, se integró el siguiente ecosistema de 13 patrones de diseño clásicos, justificando su valor técnico:

### Patrones de Comportamiento (11)
1. **State:** *Gestión del ciclo de vida.* Se implementó para controlar las transiciones del paquete (`Confirmado` -> `En Tránsito` -> `Entregado`). **Justificación:** Elimina condicionales masivos (`switch-case`) y bloquea por polimorfismo operaciones ilegales (ej. intentar entregar un paquete retenido).
2. **Strategy:** *Motor de tarifas.* Aísla las diferentes fórmulas de cobro (por peso, urgencia). **Justificación:** Cumple estrictamente con el Principio Open/Closed (OCP), permitiendo agregar nuevas lógicas comerciales dinámicamente sin modificar la entidad.
3. **Template Method:** *Estandarización de flujos.* Define el esqueleto inmutable para envíos nacionales e internacionales. **Justificación:** Fuerza a que todos los envíos pasen por controles obligatorios (validación, cobro, asignación) en un orden estricto.
4. **Visitor:** *Análisis no invasivo.* Extrae lógicas de cálculo transversales (como el `CalculoImpuestosVisitor`). **Justificación:** Permite agregar nuevos algoritmos de reportería o retenciones sin "ensuciar" las entidades con lógica analítica ajena a su núcleo.
5. **Observer:** *Sistema reactivo.* Desacopla las notificaciones (Auditoría, Notificadores). **Justificación:** Permite que los módulos de infraestructura reaccionen automáticamente a los eventos del dominio sin generar acoplamiento fuerte.
6. **Memento:** *Seguridad de estado.* Captura instantáneas inmutables del envío. **Justificación:** Esencial en logística para posibilitar *Rollbacks* seguros y consistentes ante contingencias operativas (siniestros).
...
8. **Command:** *Operaciones reificadas.* Encapsula solicitudes de negocio como objetos independientes (`IniciarViajeCommand`). **Justificación:** Facilita el encolamiento de operaciones y la ejecución del método `undo()` junto con Memento.
9. **Chain of Responsibility:** *Tubería de control.* Aplica un filtro secuencial de validaciones previas (peso, destino). **Justificación:** Falla rápido (*fail-fast*), cortando la ejecución tempranamente si hay anomalías, optimizando recursos.
10. **Iterator:** *Recorrido seguro.* Navega colecciones de objetos en memoria. **Justificación:** Expone un recorrido sobre la bodega de paquetes ocultando su representación interna de la lista al cliente.
11. **Interpreter:** *Evaluación de reglas.* Implementado mediante árbol de expresiones (`AndExpression`, `UrgenciaExpression`). **Justificación:** Permite evaluar dinámicamente combinaciones de condiciones logísticas para determinar prioridades sin hardcodear reglas complejas.

### Patrones Estructurales (2)
12. **Proxy (Virtual):** *Optimización de memoria.* Implementado para aplicar *Lazy Load* en la infraestructura. **Justificación:** Evita la saturación de memoria RAM al diferir la carga de datos pesados hasta que el sistema realmente los solicita.
...
1. **Decorator (Descartado):** Se evaluó para agregar servicios adicionales al envío (ej. seguro, embalaje especial). *Trade-off:* Se descartó porque añadir responsabilidades financieras a la entidad logística base mediante envoltorios violaba el Principio de Responsabilidad Única (SRP). Se optó por usar *Strategy*.
